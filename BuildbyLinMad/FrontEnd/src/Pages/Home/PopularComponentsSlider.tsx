...
export default function PopularComponentsSlider() {
...
buildService.getPopularComponents()
    .then((data) => {
      setComponents(data);
      ...
      <div className="carousel" ref={carouselRef} style={{ padding: '0 1.5rem 1rem 1.5rem' }}>
        {components.map((item) => (
            ...
            <div className="card">
                <div className="photo">
                <img
                    src={`http://localhost:5000/${item.imagePath || 'placeholder.jpg'}`}
                    alt={item.name || 'Componente'}
                />
                </div>
